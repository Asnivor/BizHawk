...
namespace BizHawk.Emulation.Cores.Waterbox
{
	public sealed class ElfRunner : Swappable, IImportResolver, IBinaryStateable
	{
		...
		public ElfRunner(string filename, long heapsize, long sealedheapsize, long invisibleheapsize)
		{
			...
			if (HasRelocations())
			{
				_base = PlatformLinkedLibSingleton.RunningOnUnix
					? (MemoryBlock) new MemoryBlockUnix((ulong) (orig_end - orig_start))
					: (MemoryBlock) new MemoryBlockWin32((ulong) (orig_end - orig_start));
			...
			else
			{
				...
				_base = PlatformLinkedLibSingleton.RunningOnUnix
					? (MemoryBlock) new MemoryBlockUnix((ulong) orig_start, (ulong) (orig_end - orig_start))
					: (MemoryBlock) new MemoryBlockWin32((ulong) orig_start, (ulong) (orig_end - orig_start));
				_loadoffset = 0;
				Enter();
			}
